...
func Problem11() int {
	...
	for j := 0; j < len(m); j++ {
		for i := 0; i < len(m); i++ {
			...
			if currProduct > currLargestProduct {
				currLargestProduct = currProduct
			}
		}
	}
...
func Problem12() int {
	...
	result := 0
	for i := 0; i <= limit; i++ {
		currNum := utils.GenerateTriangularNumber(i)
		currNumDivisors, _ := utils.FindAllProperDivisorsOf(currNum)
		if len(currNumDivisors) > 500 {
			result = currNum
			break
		}
	}
...
func Problem15() int {
	...
	facRow, _ := utils.FactorialBig(gridRowSize)
	facCol, _ := utils.FactorialBig(gridColumnSize)
...
func Problem18() int {
	...
	input := input_data.Problem18Input
