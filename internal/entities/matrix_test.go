...
func TestIsReturnProductOfAdjacentNumbersDiag(t *testing.T) {
	...
	// Left diagonals
	result, err := m.ReturnProductOfAdjacentNumbersDiag("left", 2, 0, 3)
	assert.Nil(t, err)
	assert.Equal(t, 32, result)
	result, err = m.ReturnProductOfAdjacentNumbersDiag("left", 3, 0, 3)
	assert.Nil(t, err)
	assert.Equal(t, 0, result)
	result, err = m.ReturnProductOfAdjacentNumbersDiag("left", 3, 0, 2)
	assert.Nil(t, err)
	assert.Equal(t, 630, result)
	result, err = m.ReturnProductOfAdjacentNumbersDiag("left", 2, 2, 3)
	assert.Nil(t, err)
	assert.Equal(t, 800, result)
	result, err = m.ReturnProductOfAdjacentNumbersDiag("left", 1, 1, 3)
	assert.Nil(t, err)
	assert.Equal(t, 6, result)
	result, err = m.ReturnProductOfAdjacentNumbersDiag("left", 3, 1, 3)
	assert.Nil(t, err)
	assert.Equal(t, 4158, result)
	result, err = m.ReturnProductOfAdjacentNumbersDiag("left", 4, 0, 3)
	assert.Nil(t, err)
	assert.Equal(t, 0, result)

	// Right diagonals
	result, err = m.ReturnProductOfAdjacentNumbersDiag("right", 4, 0, 0)
	assert.Nil(t, err)
	assert.Equal(t, 5880, result)
	result, err = m.ReturnProductOfAdjacentNumbersDiag("right", 4, 1, 1)
	assert.Nil(t, err)
	assert.Equal(t, 1470, result)
	result, err = m.ReturnProductOfAdjacentNumbersDiag("right", 1, 1, 0)
	assert.Nil(t, err)
	assert.Equal(t, 23, result)
	result, err = m.ReturnProductOfAdjacentNumbersDiag("right", 3, 1, 0)
	assert.Nil(t, err)
	assert.Equal(t, 0, result)

	// Edge cases
	result, err = m.ReturnProductOfAdjacentNumbersDiag("left", 99, 2, 3)
	assert.Nil(t, err)
	assert.Equal(t, 800, result)
	result, err = m.ReturnProductOfAdjacentNumbersDiag("left", 2, -2, -3)
	assert.Equal(t, 0, result)
	...
	assert.Equal(t, 0, result)
	assert.EqualError(t, err, "invalid 'leftOrRight' value: invalid_value")
	// Invalid adjNums value
	result, err = m.ReturnProductOfAdjacentNumbersDiag("left", -1, 0, 2)
	assert.Equal(t, 0, result)
	assert.EqualError(t, err, "invalid adjNums value: -1")
}